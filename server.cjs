...
async function createUploadSession(accessToken, file) {
  ...
  const response = await axios.post(
    ...
    `${API_BASE}/users/me/media/uploads`,
...
async function getSignedUploadUrl(accessToken, uploadId, partNumber) {
  ...
  const response = await axios.get(
    ...
    `${API_BASE}/users/me/media/uploads/${uploadId}/parts/${partNumber}/url`,
...
async function completeUploadSession(accessToken, uploadId, parts) {
  ...
  const response = await axios.patch(
    ...
    `${API_BASE}/users/me/media/uploads/${uploadId}`,
...
app.get("/env-check", (req, res) => {
  ...
  res.json({
    ...
    build: "two-app-users-me-upload-session",
    ...
    endpoints: {
      ...
      createUploadSession: `${API_BASE}/users/me/media/uploads`,

      uploadPartUrl: `${API_BASE}/users/me/media/uploads/:uploadId/parts/:partNumber/url`,

      completeUpload: `${API_BASE}/users/me/media/uploads/:uploadId`,

      createPost: `${API_BASE}/users/me/posts`
...
app.get("/oauth/callback", async (req, res) => {
  ...
  try {
    ...
    if (error) return res.status(400).send(`MidKnight OAuth error: ${error} ${error_description || ""}`);

    if (!code || !state) return res.status(400).send("Missing code/state");

    const stored = oauthStates.get(state);

    if (!stored || stored.appName !== "midknight") return res.status(400).send("Invalid MidKnight state");
...
app.listen(PORT, () => {
  ...
  console.log("TWO APP FANVUE SERVER READY - USERS ME UPLOAD BUILD");
